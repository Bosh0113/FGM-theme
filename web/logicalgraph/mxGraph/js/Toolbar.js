...
Toolbar.prototype.init = function()
{
...

	{
        ...
        //     editorUi.showDialog(new OpenNewFile(editorUi).container, 320, 250, true, true, function()
        //     {
            ...
            if(value === "Integration"){
                ...
                for(var i=0;i<LogicalModel.modelItems.length;i++){
                    ...
                    if(modelItem.name === item){
                        ...
                        select.options.remove(index-1);
                        var doc = mxUtils.parseXml(modelItem.modelDependency);
                        var dec = new mxCodec(doc);
                        dec.decode(doc.documentElement, graph.getModel());
                    }
                }
                ...
                for(var i=0;i<LogicalModel.condition.length;i++){
                    ...
                    if(condition.name === item){
                        var index = select.childElementCount;
                        select.options.remove(index-1);
                        var doc = mxUtils.parseXml(condition.modelDependency);
                        var dec = new mxCodec(doc);
                        dec.decode(doc.documentElement, graph.getModel());
                    }
                }
