...
Actions.prototype.init = function()
{
    ...
    this.addAction('UpdateTask', function()
    {
        if(window.taskID==null||window.taskID=="undefined"||window.taskID==""){
            ...
            if(Integration.length>0){
                LogicalModel.basicInfo.modelDependency = Integration;
            }else{
	...
	this.addAction('toModel',function () {
        ...
        if(cell.name!=undefined){
            ...
            if($("#viewPanel").val()=="Condition"){
                for(var i =0;i<LogicalModel.condition.length;i++){
                    var condition = LogicalModel.condition[i];
                    var item = $("#viewPanel option:selected").attr("item");
                    if(condition.name == item){
                        var encoder = new mxCodec();
                        var node = encoder.encode(graph.getModel());
                        var bmp = mxUtils.getXml(node);
                        condition.modelDependency = bmp;
                    }
                }
            }else{
                var encoder = new mxCodec();
            ...
            option.value = "Model";
            option.setAttribute("item",cell.name);
            ...
            select.onclick = function () {
				if(select.value == "Model"){
                    ...
                    // console.log(ModelDependency);
                    //cell.modelDependency = ModelDependency;
                    for(var i =0;i<LogicalModel.modelItems.length;i++){
                    	var modelItem = LogicalModel.modelItems[i];
                    	if(modelItem.name == cell.name){
                    		modelItem.modelDependency = ModelDependency;
						}
					}
				}
            };
            ...
            var curDependency = "",idx = -1;
            for(var i =0;i<LogicalModel.modelItems.length;i++){
                var modelItem = LogicalModel.modelItems[i];
                if(modelItem.name == cell.name){
                    curDependency = modelItem.modelDependency;
                    idx = i;
                    break;
                }
            }

            if(curDependency!=undefined&&curDependency.length>0){
                var doc = mxUtils.parseXml(curDependency);
                var dec = new mxCodec(doc);
                dec.decode(doc.documentElement, graph.getModel());
			}else{
                cell.geometry.x = 350;
                ...
                var modelDepen = mxUtils.getXml(node2);
                if(idx>-1){
                    LogicalModel.modelItems[idx].modelDependency = modelDepen
                }
			}
        ...
        if(cell.name!=undefined){
            ...
            select.onclick = function () {
                if(select.value == "Condition"){
                    ...
                    for(var i =0;i<LogicalModel.condition.length;i++){
                        var condition = LogicalModel.condition[i];
                        if(condition.name == cell.name){
                            condition.modelDependency = ConditionDependency;
                        }
                    }
                }
            };
            ...
            for(var i =0;i<LogicalModel.condition.length;i++){
                var condition = LogicalModel.condition[i];
                if(condition.name == cell.name){
                    curDependency = condition.modelDependency;
                }
            }
            if(curDependency!=undefined&&curDependency.length>0){
                var doc = mxUtils.parseXml(curDependency);
    ...
    this.addAction('toDataDependency',function () {
        ...
        if(cell.name!=undefined){
            ...
            var node = encoder.encode(graph.getModel());
            var bmp = mxUtils.getXml(node);
            if($("#viewPanel").val()=="Model"){
                for(var i =0;i<LogicalModel.modelItems.length;i++){
                    var modelItem = LogicalModel.modelItems[i];
                    var item = $("#viewPanel option:selected").attr("item");
                    if(modelItem.name == item){
                        modelItem.modelDependency = bmp;
                    }
                }
			}else if($("#viewPanel").val()=="Condition"){
                for(var i =0;i<LogicalModel.condition.length;i++){
                    var condition = LogicalModel.condition[i];
                    var item = $("#viewPanel option:selected").attr("item");
                    if(condition.name == item){
            ...
            select.onclick = function () {
                if(select.value == "Data"){
                    ...
                    if(index!=undefined){
                        LogicalModel.operation[index] = DataDependency
					}else {
                        LogicalModel.operation.push(DataDependency);
					};
                    //cell.modelDependency = ModelDependency;
                    /*for(var i =0;i<LogicalModel.dataItems.length;i++){
                        var dataItem = LogicalModel.dataItems[i];
                        if(dataItem.name == cell.name){
                            dataItem.modelDependency = DataDependency;
                        }
                    }*/
                }
            };
            ...
            var curDependency = "";
            for(var i=0;i<LogicalModel.operation.length;i++){
            	var operation = LogicalModel.operation[i];
            	var indexOf = "name=\""+cell.name+"\""
            	if(operation.indexOf(indexOf)>=0){
                    index = i;
            		curDependency = operation;
            		break;
				}
			}
