...
Format.prototype.refresh = function() {
	...
	if (graph.isSelectionEmpty())
	{
        ...
        mxUtils.write(label, "Logical Graph");
	...
	else
	{
		...
		if((type ==="otherdata")||(type ==="othermodel")){
            ...
            input.value =cell.name===undefined?"":cell.name;
            ...
            textArea.value = cell.desc===undefined?"":cell.desc;
            ...
            button.onclick = function(){
                var cell  = graph.getSelectionCell();
                cell.name = input.value;
                cell.desc = textArea.value;
                var obj = {
                	name :  input.value,
					desc : textArea.value
				};
                ...
                if(type==="modelitem"){
                    LogicalModel.modelItems.push(obj);
				}else if(type==="dataitem"){
                    LogicalModel.dataItems.push(obj);
				}else if(type==="condition"){
                    LogicalModel.condition.push(obj);
                }else if(type==="operation"){
                    //LogicalModel.operation.push(obj);
                }
            ...
            if((type==="dataitem")&&($("#viewPanel").val()==="Condition")){
            	...
            	for(var i=0;i<LogicalModel.dataItems.length;i++){
					(function (e) {
                        var dataItem = LogicalModel.dataItems[e];
                        ...
                        li.setAttribute("desc",dataItem.desc);
                        li.onclick = function () {
                            cell.name = li.getAttribute("value");
                            cell.desc = li.getAttribute("desc");
                            graph.cellLabelChanged(cell,li.getAttribute("value"),false);
                        }
                        ul.appendChild(li);
                    })(i);
				}
            ...
            if((type==="modelitem")&&($("#viewPanel").val()==="Condition")){
                ...
                for(var i=0;i<LogicalModel.modelItems.length;i++){
                    (function (e) {
                        var modelItem = LogicalModel.modelItems[e];
                        var li = document.createElement("li");
                        li.style.fontSize = "14px";
                        li.style.padding = "8px 5px";
                        li.style.borderBottom = "1px solid #ccc";
                        ...
                        li.setAttribute("desc",modelItem.desc);
                        li.onclick = function () {
                            cell.name = li.getAttribute("value");
                            cell.desc = li.getAttribute("desc");
                            graph.cellLabelChanged(cell,li.getAttribute("value"),false);
            ...
            if((type==="operation")&&($("#viewPanel").val()==="Data")){
                ...
                for(var i=0;i<LogicalModel.operation.length;i++){
                    (function (e) {
                    	var operation = LogicalModel.operation[e];
                        var item = $("#viewPanel option:selected").attr("item");
                        var indexOf = "name=\""+item+"\"";
                        if(operation.indexOf(indexOf)>=0){
                            ...
                            li.style.borderBottom = "1px solid #ccc";
                            li.style.listStyle = "none";
                            li.innerHTML = "Operation-"+(nIndex++);
                            li.setAttribute("value",e);
                            li.onclick = function () {
                                var index = li.getAttribute("value")*1;
                                var operationGraph = LogicalModel.operation[index];
                                var doc = mxUtils.parseXml(operationGraph);
