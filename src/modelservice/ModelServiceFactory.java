...
import enumcollection.ModelServiceLimitation;
import enumcollection.ModelServicePermission;
import enumcollection.ModelServiceStatus;
import enumcollection.Platform;
...
public class ModelServiceFactory {
	...
	public static ModelService creatModelServiceByJSON(JSONObject jMs, String ip, int port){
		...
		int msplatform = jMs.getInt("ms_platform");
		Platform platform;
		if(msplatform == 1){
		...
		int msstatus = jMs.getInt("ms_status");
		...
		int mslimit = jMs.getInt("ms_limited");
		...
		int mspermission = jMs.getInt("ms_permission");
